...
/// First-launch setup: Welcome → Language → Permissions → Model → Done.
...
struct OnboardingView: View {
    ...
    /// The tier we recommend to new users: language-aware default model.
    private var recommendedModel: ModelManager.ModelInfo {
        LanguageCatalog.defaultModel(for: AppSettings.shared.selectedLanguage)
    }
    ...
    private let permissionTimer = Timer.publish(every: 2.5, on: .main, in: .common).autoconnect()

    enum Step: Int, CaseIterable {
        case welcome, language, permissions, model, done
    }
    ...
    var body: some View {
        ...
        .onReceive(permissionTimer) { _ in
            ...
            if step.rawValue >= Step.permissions.rawValue { permissions.checkPermissions() }
        }
        .onChange(of: modelManager.activeDownloads[recommendedModel.fileName]) { newValue in
            ...
            if newValue == nil, modelManager.isModelDownloaded(recommendedModel) {
    ...
    private var languageStep: some View {
        VStack(spacing: 18) {
            stepHeading("Choose Language", "Select the language you'll dictate in.")

            VStack(spacing: 10) {
                ForEach(Language.allCases) { language in
                    Button {
                        AppSettings.shared.selectedLanguage = language
                        AppSettings.shared.selectedModel = LanguageCatalog.defaultModel(for: language).settingsId
                    } label: {
                        HStack(spacing: 12) {
                            Text(language.displayName)
                                .font(.system(size: 15))
                            Spacer()
                            if AppSettings.shared.selectedLanguage == language {
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 10)
                                .fill(AppSettings.shared.selectedLanguage == language
                                      ? Color.blue.opacity(0.12)
                                      : (colorScheme == .dark ? Color.white.opacity(0.05) : Color.white))
                        )
                        .overlay(
                            RoundedRectangle(cornerRadius: 10)
                                .stroke(AppSettings.shared.selectedLanguage == language
    ...
    private var permissionsStep: some View {
        VStack(spacing: 16) {
            stepHeading("Grant Permissions", "Lyra needs two permissions to work.")
