...
final class TextCorrector: @unchecked Sendable {
    ...
    func correct(_ text: String, language: Language = .english) -> String {
        correct(text, context: .standalone, language: language)
    }

    func correct(_ text: String, context: CorrectionContext, language: Language = .english) -> String {
        ...
        var result = text
        if language == .english && AppSettings.shared.numberConversionEnabled {
            result = convertWordsToNumbers(result)
        }
        if language.usesTechTermsPostProcessing {
            result = fixAcronymsAndTerms(result)
            result = fixCustomTerms(result)
        }
        result = fixCapitalization(result, atSentenceStart: context.atSentenceStart, useEnglishRules: language == .english)
    ...
    private func fixCapitalization(_ text: String, atSentenceStart: Bool, useEnglishRules: Bool) -> String {
        var result = text
