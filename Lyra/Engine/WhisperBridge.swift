...
final class WhisperBridge: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.lyra.whisper", qos: .userInitiated)
    ...
    /// Run a tiny dummy inference to JIT-compile Metal shaders on Apple Silicon.
    /// On Intel Macs GPU is not used, so warmup only wastes CPU/battery. Call once
    /// after model load so the first real inference isn't slower on AS.
    ...
    func warmup() async {
        guard Self.isAppleSilicon else {
            fputs("[WhisperBridge] Skipping GPU warmup on Intel\n", stderr)
            return
        }
    ...
    private func runInference(
        ...
        let langCStr = strdup(language.whisperCode)
